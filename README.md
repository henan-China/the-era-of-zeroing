...
### 传视频（分镜预览 / 成片）
- **本地视频 ≤100MB**：把文件放到仓库 `assets/`，在 `content.js` 的 VIDEOS 加 `{ ep, title, file: "文件名.mp4" }` → 页面直接播放
- **外链视频（>100MB 或平台成片）**：发布到 B 站/抖音后，在 VIDEOS 加 `{ ep, title, ext: "https://…" }` → 页面卡片跳转
- 两者可同时写：本地预览 + 平台成片链接
- 注意：GitHub 网页上传单文件限 25MB，>25MB 的视频请交给管理员用命令推送，或先压缩到 25MB 内再网页上传
